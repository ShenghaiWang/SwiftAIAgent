...
enum TraceMacroDiagnostic: String, DiagnosticMessage {
    ...
    var diagnosticID: MessageID { MessageID(domain: "SwiftAIAgentMacros", id: "TraceMacro.\(self)") }
    ...
    public static func expansion(
      ...
      in context: some MacroExpansionContext
        ...
        guard let arguments = node.arguments?.as(LabeledExprListSyntax.self),
              let trackerExpr = arguments.first(where: { $0.label?.text == "by" })?.expression else {
            return []
        }

        let messageExpr: ExprSyntax
        if let customMessageExpr = arguments.first(where: { $0.label?.text == "message" })?.expression {
